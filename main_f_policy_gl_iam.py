...
from model import  Semantic_Mapping, RL_Policy_IAM
...
def find_big_connect(image): #https://blog.csdn.net/pursuit_zhangyu/article/details/94209489 找出最大面积的连通区域
...
def main(args):
    ...
    writer = TensorboardWriter(log_dir+'tb', flush_secs=30)
    ...
    if args.eval:
        ...
        episode_success = deque(maxlen=5000)
        episode_spl = deque(maxlen=5000)
        episode_dist = deque(maxlen=5000)
    ...
    sem_frontier_loss_total = deque(maxlen=5000)

    finished = np.zeros((args.num_processes))
    wait_env = np.zeros((args.num_processes))

    g_episode_rewards = deque(maxlen=5000)

    g_value_losses = deque(maxlen=5000)
    g_action_losses = deque(maxlen=5000)
    g_dist_entropies = deque(maxlen=5000)

    per_step_g_rewards = deque(maxlen=5000)
    ...
    full_pose = torch.zeros(num_scenes, 3).float().to(device)   #x,y,朝向
    ...
    def init_map_and_pose():
        full_map.fill_(0.)
        full_pose.fill_(0.)
        full_pose[:, :2] = args.map_size_cm / 100.0 / 2.0  #地图中点
        ...
        for e in range(num_scenes):
            ...
            local_pose[e] = full_pose[e] - \
                torch.from_numpy(origins[e]).to(device).float() #计算出代理相对于其局部地图的姿态
    ...
    def init_map_and_pose_for_env(e):
        ...
        local_pose[e] = full_pose[e] - \
            torch.from_numpy(origins[e]).to(device).float() #相对于局部地图的位姿

        return local_map, full_map

    def update_intrinsic_rew(e): #通过增加平滑面积作为奖励函数
    ...
    def remove_small_points(local_ob_map, image, threshold_point, pose, name):
        ...
        dict_cost = {}
        if num==0:
            print("no frontier in remove_small_points")
        for i in range(1, len(props)):
            ...
            if props[i].area > threshold_point and dist > 50 and dist < planner.fmm_dist.max()-4:
                dict_cost[i] = cost
        maxindex = 0
        maxscores = 0
        scoreslist = []
        if dict_cost:
            ...
            tmp_Goal_point = copy.deepcopy(Goal_point)
            for i, (key, value) in enumerate(dict_cost):
                if i>=8:
                    break
                tmp_Goal_point[int(props[key].centroid[0]), int(props[key].centroid[1])] = i + 1  # 中心点

            lm = LLM(name, args.prompt_type)
            # frontier_score_list[e] = []
            reply_list[e] = None
            tpm = len(list(set(tmp_Goal_point.ravel()))) - 1
            print("number of Goal_point tpm = ", tpm)

            frontier_desc_list = []
            numofnone=0
            for lay in range(tpm):
                f_pos = np.argwhere(tmp_Goal_point == lay + 1)
                ...
                objs_list = []
                #gibson要改
                for se_cn in range(len(hm3d_category) - 1):
            ...
            query = f"Goal: Your goal is to find a {name}.\n\n"
            query += f"And you can observe the following areas containing objects while exploring a house:\n\n"
            for idx, desc in enumerate(frontier_desc_list):
                query += f"- Area {idx}: {desc}\n\n"

            if len(frontier_desc_list) == 0:
                query += "No current candidate frontiers in this map.\n\n"
            ...
            while tpm != 0:
                try:
                    if len(frontier_desc_list) == 0:
                        answer_rank = [0, 1, 2, 3]
                        index = 0
                    elif numofnone == tpm:  # 赋值为0.5
                        answer_rank = [0, 1, 2, 3]
                        index = 0
                        maxscores = 0.5
                    else:
                        answer_rank, reply, index, maxscores, scoreslist = lm.choose_frontier(query, 1)
                        print("scoreslist", maxscores)
            ...
            if tpm == 0:
                reply = "Answer is No current frontiers.\n\n"
                index = 0
                answer_rank = [0, 1, 2, 3]
            ...
            if answer_rank == -1:
                try:
                    answer_rank = random.randint(0, tpm - 1)
                except:
                    answer_rank = 0

            print("answer rank", answer_rank)

            order_num = 0  # 新的次序 计数
            for x in answer_rank:  # 剔除掉没有选上的索引 重新排序
                for i, (key, value) in enumerate(dict_cost):
                    if x == i:
                        if x == index:
                            maxindex = order_num
                        ...
                        order_num += 1
                        # print("remove", i, key, value, order_num)
                        Goal_edge[img_label == key + 1] = 1  # 周围所有的点
                        # print("Goal_edge", Goal_edge)
                        Goal_point[int(props[key].centroid[0]), int(props[key].centroid[1])] = order_num  # 中心点
                        # print("Goal_point", np.nonzero(Goal_point))
                        Goal_score.append(value)

        return Goal_edge, Goal_point, maxindex, maxscores, scoreslist
    ...
    # g_action 采样出来的动作和g_action_log_prob该动作的对数概率
    ...
    #print("g_action: ", g_value, g_action, g_action_log_prob, g_rec_states)
    ...
    #print("-----------planner_inputs for frame 1" + str(planner_inputs))

    for e, p_input in enumerate(planner_inputs): #e是索引 字典形式储存{'new_goal': 1, 'found_goal': 0}
    ...
    for step in range(args.num_training_frames // args.num_processes + 1):
        ...
        # ------------------------------------------------------------------
        greedy = random.uniform(0, 10)
        greedy_threshold = args.greedy
        # ------------------------------------------------------------------
        # Global Policy 根据代理的行为和环境的反应来更新和维护局部与全局地图信息以及细节的过程，同时处理新的剧集（即仿真环境的新开始）和特定情况（如清理障碍）。
        if l_step == args.num_local_steps - 1:
            ...
            for e in range(num_scenes):
                ...
                local_ob_map[e] = cv2.dilate(_local_ob_map, kernel) #膨胀操作，以便更清晰地识别障碍区域

                show_ex = cv2.inRange(local_map[e][1].cpu().numpy(),0.1,1) #提取可探索区域的轮廓
                
                contours,_=cv2.findContours(show_ex, cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
                if len(contours)>0:
                    contour = max(contours, key = cv2.contourArea)
                    cv2.drawContours(local_ex_map[e],contour,-1,1,1) #轮廓绘制到local_ex_map上
                ...
                target_edge = local_ex_map[e]-local_ob_map[e] #之间的差值来确定可探索的边界区域

                target_edge[target_edge>0.8]=1.0  #边界
                ...
                #L3MVN
                target_edge_map[e], target_point_map[e], actionindex, maxscore, new_scorelist  = remove_small_points(_local_ob_map, target_edge, 4, local_pose_map,cname)
            ...
            if maxscore > 0.5:
                ...
                if greedy < greedy_threshold:  # 使用LLM来决策 将LLM的结果和RL结果进行对齐
                    g_value, g_action, g_action_log_prob, g_rec_states = \
                        g_policy.act_alignwithLLM(
                            g_rollouts.obs[g_step + 1],
                            g_rollouts.rec_states[g_step + 1],
                    ...
                    print(greedy_threshold,"test是否一致:",g_action==actionindex)
                else:  # 使用强化学习来决策
                    g_value, g_action, g_action_log_prob, g_rec_states = \
                        g_policy.act(
                            g_rollouts.obs[g_step + 1],
            ...
            ##### select randomly point 选点前初始化global goal
            # ------------------------------------------------------------------
            actions = torch.randn(g_action.shape[0], 2)*6 #正态分布
        ...
        for e in range(num_scenes):
            ...
            if len(new_scorelist) > 0:
                llm_reward = new_scorelist[int(global_item)]
                llm_reward = llm_reward - 0.5
                print("llm_reward", llm_reward)
        ...
        if (step * num_scenes) % args.save_interval < \
                num_scenes:
            if len(g_episode_rewards) >= 1000 and (np.mean(episode_success) >= 0.33) and not args.eval:
                num_best_model += 1
                print("save num_best_model", num_best_model)
                l = "save num_best_model" + str(num_best_model)
                logging.info(l)
                torch.save(g_policy.state_dict(),
                           os.path.join(log_dir, f"{num_best_model}_model_best_{np.mean(episode_success)}_{np.mean(episode_spl)}_{np.mean(episode_dist)}_{step * num_scenes}.pth"))
                best_g_reward = np.mean(g_episode_rewards)
                best_episode_success = np.mean(episode_success)
