...
class RL_Policy_IAM(nn.Module):
    ...
    def act(self, inputs, rnn_hxs, masks, extras=None, deterministic=False, invalid_action_masks=None):
        ...
        #多分类问题logits表示每个样本的log probabilities(对数概率) 对数概率（log probability）是指一个事件发生的概率的对数
        ...
        if invalid_action_masks is not None:
            ...
            dist = Categorical(logits=actor_features) #每一个数据都减去其对数指数累加和


        action = dist.sample()  #分布中采取样本https://deepinout.com/pytorch/pytorch-questions/60_pytorch_understanding_log_prob_for_normal_distribution_in_pytorch.html
        print("act______________action", action)
        action_log_probs = dist.log_prob(action) #该样本在分布中的对数概率

        return value, action, action_log_probs, rnn_hxs

    def act_alignwithLLM(self, inputs, rnn_hxs, masks, LLManction, device, extras=None, deterministic=False,
        ...
        if invalid_action_masks is not None:
            ...
            dist = Categorical(logits=actor_features)  # 每一个数据都减去其对数指数累加和
        list = []
        list.append(LLManction)
        action = torch.Tensor(list).to(device)
        print("act_alignwithLLM______________action", action)
        action_log_probs = dist.log_prob(action)  # 该样本在分布中的对数概率
        # print("act_alignwithLLM______________action_log_probs", action_log_probs)
        return value, action, action_log_probs, rnn_hxs
